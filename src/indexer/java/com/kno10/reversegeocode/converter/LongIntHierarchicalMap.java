...
class LongIntHierarchicalMap {
	...
	// Chunked maps.
	Int2ObjectOpenHashMap<Short2IntOpenHashMap> topmap = new Int2ObjectOpenHashMap<Short2IntOpenHashMap>();
	...
	public boolean containsKey(long id) {
		...
		Short2IntOpenHashMap chunk = topmap.get(prefix);
	 ...
	 */
	public int getOrDefault(long id, int notfound) {
		int prefix = (int) (id >>> shift);
		if (maxid > 0L && id >= maxid) {
			throw new RuntimeException(
	...
	public void put(long id, int val) {
		...
		Short2IntOpenHashMap chunk = topmap.get(prefix);
		if (chunk == null) {
			chunk = new Short2IntOpenHashMap();
	...
	public int computeSize() {
		int size = 0;
		for (Short2IntOpenHashMap m : topmap.values()) {
			size += m.size();
		}
		return size;
	}
